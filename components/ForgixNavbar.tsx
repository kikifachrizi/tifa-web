...
interface ForgixNavbarProps {
    theme?: 'forgix' | 'tifa';
}

export default function ForgixNavbar({ theme = 'forgix' }: ForgixNavbarProps) {
            ...
            >
                <div className="max-w-7xl mx-auto px-6 lg:px-12 flex items-center justify-between">
                    ...
                    <div className="hidden lg:flex items-center gap-1">
                        {navItems.map((item) => (
                            <Link
                                ...
                                href={item.href}
                                onClick={(e) => {
                                    // Handle hash links for both Forgix (/#) and TIFA (/tifa#) pages
                    ...
                    <div className="flex items-center gap-3">
                        {/* Contact Button - WhatsApp (only for Forgix theme) */}
                        {!isTifa && (
                            <a
                                href="https://wa.me/6281234567890?text=Halo%20Forgix!"
                                target="_blank"
                        ...
                        <Link
                            href="/login"
                            className={`hidden md:block px-4 py-2 text-sm font-medium transition-colors ${isScrolled && !isTifa
                                ? 'text-[#2C3A50] hover:text-[#2FAEB7]'
                        ...
                        <Link
                            ...
                            className={`hidden md:block px-5 py-2.5 text-sm font-semibold rounded-lg transition-all hover:-translate-y-0.5 ${isTifa
                                ...
                                : isScrolled
                                    ? 'border-2 border-[#2FAEB7] text-[#2FAEB7] hover:bg-[#2FAEB7] hover:text-white'
                                    : 'border-2 border-white/50 text-white hover:bg-white/10'
                                }`}
                        >
                            {isTifa ? 'Get started' : 'Sign Up'}
                        </Link>
                ...
                >
                    <div className="p-6 pt-20">
                        ...
                        <div className="mt-8 pt-8 border-t border-gray-200/20 space-y-3">
                            <Link
                                href="/login"
                                className={`block w-full px-4 py-3 text-center text-base font-medium rounded-lg transition-colors ${isTifa
                                    ? 'text-white/80 hover:text-white hover:bg-white/10'
                                    : 'text-[#2C3A50] hover:bg-gray-100'
                                    }`}
                                onClick={() => setIsMobileMenuOpen(false)}
